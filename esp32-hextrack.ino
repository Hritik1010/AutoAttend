...
// lastSentAt: last time we POSTed this hex to HexTrack (seconds) used for dedupe
...
void sendToAutoAttend(const std::string &hexValue, bool detected) {
  ...
  while (retries < MAX_RETRIES) {
    ...
    // Build AutoAttend payload: {"uuid":"D7E1A3F4", "hex_value":"...", "detected":true}
    String payload = String("{\"uuid\":\"") + String(TARGET_UUID) + 
...
class MyAdvertisedDeviceCallbacks : public BLEAdvertisedDeviceCallbacks {
  void onResult(BLEAdvertisedDevice advertisedDevice) override {
    ...
    // Process only if target UUID matches
    if (matches) {
      if (devicesWithTarget.insert(addr).second) {
        ...
        if (advertisedDevice.haveServiceData()) {
          try {
            ...
            if (!sData.empty()) {
              ...
              // If ASCII part itself looks like a hex string, treat it as payload
              if (isAsciiHexString(ascii)) {
                ...
                if (presentDevices.find(h) == presentDevices.end()) {
                  // Not present yet -> mark present and send check-in
                  presentDevices.insert(h);
                  sendToAutoAttend(h, true);  // detected = true (check-in)
                }
              } else {
        ...
        // --- Parse AD structures to find Local Name ---
        if (payloadLength > 0) {
          int idx = 0;
          ...
          while (idx < payloadLength) {
            ...
            if (len == 0) break;
            if (idx + len >= payloadLength) break;
            ...
            if (type == 0x08 || type == 0x09) {
              ...
              if (nameLen > 0) {
                ...
                // If the local name itself is an ASCII hex string, handle detection
                if (isAsciiHexString(name)) {
                  ...
                  if (presentDevices.find(h) == presentDevices.end()) {
                    presentDevices.insert(h);
                    sendToHexTrack(h, true);  // detected = true (check-in)
                  }
                }
                foundLocalName = true;
              }
        ...
        // Extract payload data after BLE header
        const int MINIMUM_HEADER_SIZE = 24;
        if (payloadLength > MINIMUM_HEADER_SIZE) {
          std::string payloadData = hexData.substr(MINIMUM_HEADER_SIZE * 2);
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("Starting AutoAttend BLE Scanner (scanning for D7E1A3F4)...");
...
void loop() {
  ...
  int scanTime = 2;  // scan for 2 seconds
  ...
  for (const auto &h : presentDevices) {
    ...
    if ((nowSec - it->second) > PRESENCE_TIMEOUT_SECONDS) {
      ...
      Serial.printf("Device %s timed out (no longer seen). Sending check-out...\n", h.c_str());
      sendToAutoAttend(h, false);  // detected = false (check-out)
